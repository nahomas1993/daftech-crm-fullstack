...
public class AgreementTypeService : IAgreementTypeService
{
    ...
    private static AgreementTypeDto ToDto(AgreementType t) => new(t.Id, t.Name, t.Description, t.IsSystemDefined);
    ...
    public async Task<IReadOnlyList<AgreementTypeDto>> GetAllAsync(CancellationToken ct = default) =>
        await _db.AgreementTypes.AsNoTracking().OrderBy(t => t.Name)
            .Select(t => new AgreementTypeDto(t.Id, t.Name, t.Description, t.IsSystemDefined)).ToListAsync(ct);
    ...
    /// <summary>Only Description is editable — Name is not, since AgreementService and the training-before-support gate resolve Support/Training by name; renaming could silently break that gate for existing types.</summary>
