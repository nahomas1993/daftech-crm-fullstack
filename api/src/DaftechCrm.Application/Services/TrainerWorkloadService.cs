...
/// signal), and each open training assignment counts as 1.5 — training
/// is typically a bigger, less interruptible time commitment than a
/// single ticket, and stays "open" (counted) for as long as the
/// SystemProduct it's on hasn't been marked training-Completed, since the
/// trainer may still need to log more sessions against it. These weights
/// are a deliberately simple, transparent heuristic (not a tuned model) —
/// Admin sees the underlying counts too, not just the score, specifically
/// so a human can sanity check or override the ranking.
/// </summary>
public class TrainerWorkloadService : ITrainerWorkloadService
{
    ...
    private const double OpenTrainingWeight = 1.5;
    ...
    private async Task<IReadOnlyList<TrainerWorkloadDto>> RankTrainersAsync(CancellationToken ct)
    {
        ...
        var openTrainingCounts = await _db.TrainingAssignments
            .AsNoTracking()
            .Where(ta => trainerIds.Contains(ta.TrainerEmployeeId) &&
                         ta.SystemProduct.TrainingCompletionStatus != TrainingCompletionStatus.Completed)
        ...
        var provisional = trainers.Select(trainer =>
        {
            ...
            var openTraining = openTrainingCounts.GetValueOrDefault(trainer.Id, 0);

            var score = active + (pending * PendingWeight) + (highPriority * HighPriorityBonus) +
                        (overdue * OverdueBonus) + (openTraining * OpenTrainingWeight);

            return (trainer.Id, trainer.FullName, active, pending, highPriority, overdue, openTraining, score);
        }).ToList();
        ...
        return provisional.Select(p =>
        {
            ...
            return new TrainerWorkloadDto(
                p.Id, p.FullName, p.active, p.pending, p.highPriority, p.overdue, p.openTraining, p.score, isExcessive
            );
        })
