...
public class TrainerWorkloadService : ITrainerWorkloadService
{
    ...
    public async Task<TrainerAssignmentRecommendationDto> GetEligibleTrainersAsync(CancellationToken ct = default)
    {
        ...
        var activeTrainingCounts = await _db.TrainingSessions
            .AsNoTracking()
            .Where(ts => ts.TrainerEmployeeId != null && trainerIds.Contains(ts.TrainerEmployeeId.Value) &&
                         ActiveTrainingStatuses.Contains(ts.CompletionStatus))
            .GroupBy(ts => ts.TrainerEmployeeId!.Value)
        ...
        var results = provisional.Select(p =>
        {
